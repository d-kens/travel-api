...
class TourListTest extends TestCase
{
    /*
    Scenarios to test
        - Whether pagination is returned correctly.
        - Whether the price is returned correctly with censt
        - Whether the Route-Mpdel works correctly using slugs and filters record by slug
    ...
    */
    ...
    public function test_tours_list_sorts_by_price_correctly(): void
    {
        ...
        $response = $this->get('/api/travels/'.$travel->slug.'/tours?sortBy=price&sortOrder=asc');
    ...
    public function test_tours_list_filters_by_price_correctly(): void
    {
        ...
        $endpoint = '/api/travels/'.$travel->slug.'/tours';
    ...
    public function test_tour_list_retuens_validation_errors():void
    {
        $travel= Travel::factory()->create();

        $response = $this->getJson('/api/travels/'.$travel->slug.'/tours?dateFrom=abcde');
        $response->assertStatus(422);

        $response = $this->getJson('/api/travels/'.$travel->slug.'/tours?priceFrom=abcde');
        $response->assertStatus(422);
    }
}
